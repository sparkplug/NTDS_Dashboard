...
_filterRowsBy(filterBy) {
    ...
    var parish=this.state.parish;
    var subcounty=this.state.subcounty;
    ...
    if (_.isEmpty(this.state.rows)){
        ...
        if(subcounty)
        {
            filteredRows=_.where(filteredRows,{subcounty:subcounty})
        }
        if(parish)
        {
            filteredRows=_.where(filteredRows,{parish:parish})
        }
...
_districtFilter(e) {
    ...
    if (e.target.value==="0"){
        ...
        currentFilterStore.setCurrentDistrict(district);
    }



    this.setState({ district: e.target.value });
    this._filterRowsBy({name:"district",value:district});
...
_statusFilter(e){
    ...
    this._filterRowsBy({name:"status",value:e.target.value});
...
render()
{
    ...
    let statusFilters=[{id:"0",name:"Filter By Status"},{id:"Eligible For Interview",name:"Eligible For Interview"},{id:"Disqualified for Interview",name:"Disqualified for Interview"},{id:"Eligible For Training",name:"Eligible For Training"},{id:"Disqualified for Training",name:"Disqualified for Training"}];
...
<CardTitle title="Reporters"  subtitle={this.state.filteredRows.length + " "+"reporters"}/>
<Toolbar>
