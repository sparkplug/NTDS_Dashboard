...
let View_district = React.createClass({
    propTypes: {
...
getStateFromStores(){
    ...
    var district = this.context.router.getCurrentParams().id;
    var districts = DistrictStore.getAll();
    if (district){
        districts = DistrictStore.filter({district:district});
...
componentDidUpdate(prevProps){
    ...
    if(oldId !== newId){
        var districts = DistrictStore.filter({district:newId});

        this.setState({rows:districts,filteredRows:districts,district:newId.toUpperCase()});
...
<CardTitle title={this.state.district}  />
<Toolbar>
